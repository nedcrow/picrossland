    ...
    IEnumerator FireBaseDefaultInstanceSetting() {
        ...
        while (time<checkTime) {
            if (NetworkConnectionChecker.instance.success == true)
            {
                ...
                DatabaseReference reference = FirebaseDatabase.DefaultInstance.RootReference; Debug.Log("FireBaseDefaultInstanceSetting");
                break;
            }
            time += 0.01f;
...
//    void OnLoadLand() { }
    ...
    public void OnSaveAdmin(bool afterClear)
    {
        if(afterClear == false) { OnLoadAdmin(); }//최초값으로 저장되는 것을 막는 역할.
        ...
        SaveLocal_Game("UserData.save"); //LocalDB  "lastLand", "gotLandList", "lastpuzzle", "ingPuzzleList"
    ...
    public void LoadSetting() {
        ...
        try
        {
            ...
            {   
                ...
                Debug.Log(UserManager.Instance.currentUser.settingVal[0]);
    ...
    public bool LoginDataCheck()
    {
        ...
        try
        {
            ...
            if (fi.Exists)
            {
                ...
                string loginData = bf.Deserialize(fs).ToString();
                fs.Close();
